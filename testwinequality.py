...
def save_single_file(dataframe: DataFrame, output_path: str):
    ...
    temp_path = "s3://winequalityprediction/datasets/temp_test_data"
    dataframe.coalesce(1).write.mode("overwrite").csv(temp_path, header=True, sep=";")

    s3 = boto3.resource('s3')
    bucket_name ="winequalityprediction"
...
if __name__ == "__main__":
    try:
        ...
        # Command-line arguments or default S3 paths
        input_path = sys.argv[1] if len(sys.argv) > 1 else "s3://winequalityprediction/datasets/TrainingDataset.csv"
        valid_path = sys.argv[2] if len(sys.argv) > 2 else "s3://winequalityprediction/datasets/ValidationDataset.csv"
        model_output_path = sys.argv[3] if len(sys.argv) > 3 else "s3://winequalityprediction/models/winemodel"
        ...
        # Save test dataset to S3 as a single file
        logger.info("Saving test dataset to S3 as a single file...")
        save_single_file(test_data, "s3://winequalityprediction/datasets/TestDataset.csv")
